<x-card>
    ...
    <form action="{{ route('films.update', $film) }}" method="POST">
        ...
        <div>
            ...
            <x-input id="title" type="text" name="title" :value="old('title') ?? $film->title" required
        ...
        <div class="my-4">
            ...
            <x-input id="year" type="number" min="1895" max="3000" step="1" name="year"
                     :value="old('year') ?? $film->year" required class="block mt-1 w-full"/>
        </div>
        ...
        @foreach(\App\Models\Job::all() as $job)
            <div class="my-4">
                ...
                <select name="contacts[{{$job->id}}][]" multiple="multiple"
                    ...
                    @foreach($film->contacts()->where('job_id', $job->id)->get() as $contact)
                        <option value="{{ $contact->id }}" selected>{{ $contact->getFullName() }}</option>
                    @endforeach
                </select>
            </div>
        @endforeach
